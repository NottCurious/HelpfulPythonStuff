...
import discord
from ..backend_stuff import easy_embed as ezembed
import numpy as np
...
def quote_dict_to_embed(quote: dict) -> discord.Embed:
    """Changes the quote dict to a discord.Embed

    Args:
        quote (dict): The dict of the quote

    Returns:
        discord.Embed: The discord.Embed contianing the quote
    """
...
def get_random_quote_dict() -> dict:
    """Get's a random quote, returns in a dict format
...
def get_last_quote() -> discord.Embed:
    """Returns the last quote saved

    Returns:
        discord.Embed: The last quote saved embedded in a discord.Embed
    """
    log.debug(f"Opening JSON File")
    with open("./data/json/quotes.json", "r") as file:
        log.debug(f"Loading JSON file")
...
def get_number_of_quotes() -> int:
    """Returns the number of quotes currently stored in the file

    Returns:
        int: the number of quotes stored in the file
    """
    log.debug(f"Opening JSON File")
    with open("./data/json/quotes.json", "r") as file:
        log.debug(f"Loading JSON file")
